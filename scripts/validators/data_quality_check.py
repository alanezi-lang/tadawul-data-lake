"""
tadawul-data-lake | data_quality_check.py (v2)
...
def check(issues, level, check_name, message, **kwargs):
    ...
    log.log(logging.ERROR if level == "CRITICAL" else
...
def check_external(issues):
    ...
    log.info(f"✅ الخارجي: Brent={row.get('brent_crude_close','N/A')} | "
             f"VIX={row.get('vix_close','N/A')} | "
             f"TASI={row.get('tasi_close','N/A')}")
...
def check_saibor(issues):
    ...
    today = df[df["date"] == TODAY] if "date" in df.columns else pd.DataFrame()
    if today.empty:
        check(issues, "INFO", "saibor_today", "لا بيانات SAIBOR لليوم (قد يكون API ساما متأخر)")
    elif pd.isna(today.iloc[0].get("saibor_3m")):
        check(issues, "INFO", "saibor_3m_null",
              "SAIBOR 3M = None (URL ساما قد تغيّر - راجع يدوياً)")
    else:
...
def check_foreign_ownership(issues):
    ...
    if not path.exists():
        check(issues, "INFO", "foreign_file",
              "ملف ملكية الأجانب غير موجود بعد (متوقع في أول أسبوع)")
        return
